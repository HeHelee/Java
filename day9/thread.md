...
#### WAITING 상태에서 join() 메서드와 wait() 메서드의 차이
- join() 메서드와 wait() 메서드는 RUNNABLE 상태의 쓰레드를 무한정 WAITING 시킨 다는 점에서 동일하지만 사용하는 용도는 다르다.
- join() 메서드는 특정 객체의 쓰레드가 종료될때까지를 기다리지만 wait() 메서드는 특정 조건이 총족될때까지 기다린다.
- 그리고 join() 메서드는 그냥 호출이 가능하지만 wait() 메서드는 동기화 블록안에서만 가능하다.
- 또한 깨어나는 조건도 다르다. join() 메서드는 해당 쓰레드가 종료되면 깨어나지만, wait() 메서드는 notify()를 호출해야한다.
